...
import { accountTypeForRole } from '../../utils/roles'
...
export default function DashboardTopbar({ title = 'Dashboard', onMenuClick }) {
  ...
  const base = accountTypeForRole(profile?.role) === 'engineer' ? '/engineer' : '/client'
  ...
  return (
    <header className="sticky top-0 z-30 bg-white border-b border-ink-100 h-16 flex items-center justify-between px-4 sm:px-6">
      ...
      <div className="flex items-center gap-4">
        <Link to={`${base}/messages`} className="relative p-2 text-ink-700 hover:text-brand-600">
          <MessageSquare size={20} />
        </Link>
        <Link to={`${base}/notifications`} className="relative p-2 text-ink-700 hover:text-brand-600">
          <Bell size={20} />
          {hasUnread && <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-red-500" />}
        </Link>
